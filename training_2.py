...
# Modification date: Wed Dec 29 16:42:24 2021
...
try:
    ground = open("ground_training2.csv", "w", encoding = "utf-8")
    gwriter = csv.writer(ground)
    gtableau = []
    water = open("water_training2.csv", "w", encoding = "utf-8")
    wwriter = csv.writer(water)
    wtableau = []

    #writer.writerow(row)

    originals = os.listdir("C:\\Users\\emirh\\OneDrive\\Bureau\\My_World_Dont_Change\\dosyalar\\Kodlarim\\Python\\imgedit\\imgedit_training\\originals2\\")
    edited = os.listdir("C:\\Users\\emirh\\OneDrive\\Bureau\\My_World_Dont_Change\\dosyalar\\Kodlarim\\Python\\imgedit\\imgedit_training\\edited2\\")

    print(edited)
    print(originals)

    for i in range(len(originals)):
        eimg = Image.open(f"C:\\Users\\emirh\\OneDrive\\Bureau\\My_World_Dont_Change\\dosyalar\\Kodlarim\\Python\\imgedit\\imgedit_training\\edited2\\{edited[i]}")
        oimg = Image.open(f"C:\\Users\\emirh\\OneDrive\\Bureau\\My_World_Dont_Change\\dosyalar\\Kodlarim\\Python\\imgedit\\imgedit_training\\originals2\\{originals[i]}")
    ...
    gtableau = set(gtableau) - set(wtableau)
    gtableau = list(gtableau)
    wtableau = set(wtableau) - set(gtableau)
    wtableau = list(wtableau)
